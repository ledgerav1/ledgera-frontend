...
function getJwtSecret(): string | null {
  ...
  // Next route modules often run with cwd = ledgera-frontend/, so try a few locations.
  const candidates = [
...
export async function GET(
  ...
  const url = `${backendUrl}/analytics/${companyId}/leakage-score`;
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      {
        error: "Backend leakage-score fetch failed",
        status: res.status,
        detail: text,
      },
      { status: res.status }
  ...
  const data = (await res.json()) as LeakageScoreResponse;
