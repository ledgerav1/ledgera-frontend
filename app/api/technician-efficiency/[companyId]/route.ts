...
export async function GET(
  ...
  // Keep aligned with backend default windowDays=30 used elsewhere
  const url = `${backendUrl}/analytics/${companyId}/technician-efficiency?windowDays=30`;
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      {
        error: "Backend technician-efficiency fetch failed",
        status: res.status,
        detail: text,
      },
      { status: res.status }
  ...
  const data = (await res.json()) as TechnicianEfficiencyResponse;
